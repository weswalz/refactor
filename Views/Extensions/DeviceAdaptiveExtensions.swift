...
extension DeviceEnvironment {
    ...
    var networkConfig: String {
        ...
        return "\(settings.preferredConnectionType.displayName) - Timeout: \(Int(settings.oscTimeout))s"
    }
}
...
struct DeviceAdaptiveTextModifier: ViewModifier {
    ...
    private func font(for style: DeviceTextStyle) -> Font {
        switch (style, deviceEnvironment.deviceType) {
        case (.title, .iPhone):
            return .title2
        case (.title, .iPad):
            return .largeTitle
...
struct DeviceAdaptiveSpacing {
    let deviceEnvironment: DeviceEnvironment
    
    init(deviceEnvironment: DeviceEnvironment) {
